...
public class LoadAndPlayCont : MonoBehaviour
{
    ...
    public bool EnqueuPaths(string path, string pc_name)
    {
        //Loading new paths -> first clear everything
        pc_paths.Clear();
        pc_buff.Clear();
        string fold_path = Application.dataPath + "/" + path + pc_name;
        DirectoryInfo dir = new DirectoryInfo(fold_path);
        ...
        {
            string temp = p.ToString().Replace("\\", "/");
            //Debug.Log(temp);
            //Debug.Log(temp.Substring(temp.Length - 2 * pc_name.Length - 10));
            string pcd_path = temp.Substring(temp.Length - 2*pc_name.Length - 10 , 2*pc_name.Length + 6);
            //Debug.Log(pcd_path);
